...
export async function getMenuItems() {
  return new Promise((resolve) => {
    db.transaction((tx) => {
      //tx.executeSql( 'delete  from menuitems', [],(_, { rows: { _array } }) => {Alert.alert("byeRows"); });
      tx.executeSql('select uuid as id,title,price,category from menuitems', [], (_, { rows }) => {
        resolve(rows._array);
      });
    });
  });
...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
   ...
   {/**  const columns = ['uuid', 'title', 'price', 'category'];
    ...
    `;
    tx.executeSql(sqlStatement, [], (_, resultSet) => {
      // Success callback
      console.log('Menu items saved successfully:', resultSet);
    },
    (_, error) => {
      // Error callback
